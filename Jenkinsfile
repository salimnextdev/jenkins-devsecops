pipeline {
  ...
  tools { 
   ...
   stages{
	...
	stage('Kubernetes Deployment of Salim Buggy Web Application') {
	   steps {
	...
	stage('RunDASTUsingZAP') {
          steps {
		    withKubeConfig([credentialsId: 'kubelogin']) {
				sh('zap.sh -cmd -quickurl http://$(kubectl get services/salimnextdev --namespace=devsecops -o json| jq -r ".status.loadBalancer.ingress[] | .hostname") -quickprogress -quickout ${WORKSPACE}/zap_report.html')
